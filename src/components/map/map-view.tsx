...
/* Light basemap. Uses standard OpenStreetMap tiles by default, or CARTO basemaps if NEXT_PUBLIC_CARTO_API_KEY is configured. */
const CARTO_KEY = process.env.NEXT_PUBLIC_CARTO_API_KEY;
const TILE_URL = CARTO_KEY
    ? `https://{s}.basemaps.cartocdn.com/rastertiles/voyager_nolabels/{z}/{x}/{y}{r}.png?key=${CARTO_KEY}`
    : "https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png";
const TILE_URL_LABELS = CARTO_KEY
    ? `https://{s}.basemaps.cartocdn.com/rastertiles/voyager_only_labels/{z}/{x}/{y}{r}.png?key=${CARTO_KEY}`
    : null;
...
export default function MapView({
    ...
    return (
        <div className={cn("relative isolate z-0 h-full w-full", className)}>
            ...
            >
                <TileLayer url={TILE_URL} attribution={ATTRIBUTION} subdomains="abcd" maxZoom={19} />
                {TILE_URL_LABELS && <TileLayer url={TILE_URL_LABELS} subdomains="abcd" maxZoom={19} opacity={0.9} />}
                <Ready onReady={onReady} />
                <ResizeWatcher />
                {center && !fitTo && <CenterOn center={center} zoom={zoom} />}
                {fitTo && <FitBounds points={fitTo} padding={fitPadding} paddingBottom={paddingBottom} paddingTop={paddingTop} />}
